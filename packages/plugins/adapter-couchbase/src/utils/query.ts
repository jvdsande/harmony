function isObject(value: any) /* : boolean %checks */ {
  return typeof value === 'object' && !Array.isArray(value) && value !== null
}

function _toMongoDottedObject(
  obj : Record<string, any>, target: Record<string, any> = {}, path : string[] = [], filter = false,
) : Record<string, any> {
  ...
  objKeys.forEach(key => {
    if (key.startsWith('$')) {
      const val = Array.isArray(obj[key])
        ? obj[key].map((v : Record<string, any>) => _toMongoDottedObject(v, {}, [], filter))
    ...
    } else if (Object(obj[key]) === obj[key] && (!Array.isArray(obj[key]) || filter)) {
